...
Функция ПроверитьПакет(ИмяПакета)
	...
	Если Не _ХранилищеЗависимыхРепозиториев.ЕстьДанныеПоПакету(ИмяПакета) Тогда
		РезультатПроверки.ТекстСообщения = "Данные по пакету пока отсутствуют. Сбор информации поставлен в очередь - результаты будут доступны позже.";
...
Процедура ОтветБейдж(ИмяПакета, РезультатПроверки, Ответ)
	...
	Ответ.ТелоДвоичныеДанные = ПолучитьДвоичныеДанныеБейджа(
		КоличествоЗависимыхРепозиториев, 
...
Функция ПолучитьДвоичныеДанныеБейджа(КоличествоЗависимыхРепозиториев, ТекстСообщения)
	...
	Если ЗначениеЗаполнено(ТекстСообщения) Тогда
		...
		Значение = СтрШаблон(ШаблонОшибка, ТекстСообщения);
