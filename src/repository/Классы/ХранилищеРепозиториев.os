...
Процедура УдалитьПоПолномуИмени(ПолноеИмя) Экспорт
	...
	СущностьРепозиторий = _МенеджерСущностей.ПолучитьОдно(Тип("СущностьРепозиторий"), ОпцииПоиска);
	Если Не СущностьРепозиторий = Неопределено Тогда
		_МенеджерСущностей.Удалить(СущностьРепозиторий);
	КонецЕсли;
