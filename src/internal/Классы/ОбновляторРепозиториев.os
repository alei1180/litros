...
Процедура ОбновитьРепозитории(ПолныеИменаРепозиториев) Экспорт
	...
	Для Каждого Элемент Из ДанныеОтвета["data"] Цикл
		...
		ОбработанныеРепозитории.Добавить(ПолноеИмя);

	КонецЦикла;

	Для Каждого ПолноеИмя Из ПолныеИменаРепозиториев Цикл
		Если Не ОбработанныеРепозитории.Содержит(ПолноеИмя) Тогда
			_ХранилищеРепозиториев.УдалитьПоПолномуИмени(ПолноеИмя);
		КонецЕсли;
	КонецЦикла;
