...
- через переменную окружения `litros.github.Токен=ВАШ-ТОКЕН`
- или в файл `autumn-properties.json` (создайте его в корне проекта, если отсутствует):
...
Токен `GitHub` можно передать и через переменную окружения: `litros.github.Токен=ВАШ-ТОКЕН`.
