...
Функция СоздатьЭлементВыдачи(НастройкиТеста, НомерЭлемента, КоличествоРепозиториев)
	...
	НомерРепозитория = ?(
		ЭтоНерелевантнаяВыдача,
		0,
		Число(СтрЗаменить(СтрЗаменить(ИмяРепозитория, "repo", ""), "fork", "")));
	НачальныйИдентификатор = ?(НастройкиТеста.ТолькоФорки, 2000000, 1000000);
	Элемент["repository"]["id"] = НачальныйИдентификатор + НомерРепозитория;
	Элемент["repository"]["node_id"] = "node-" + ИмяРепозитория;
